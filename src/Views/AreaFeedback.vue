<template>
  ...
  >
    ...
    <div class="flex-grow flex items-center justify-center px-4 sm:px-8 py-10">
      ...
      >
      <!-- Aqui é o nome da area que vai ser dinamico -->
     <h2 v-if="area" class="text-2xl sm:text-3xl font-extrabold text-[#004E7D]">
  {{ area.nome }}
</h2>
        ...
        <form @submit.prevent="enviarFeedback" class="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              class="border border-[#009FE3]/30 focus:border-[#009FE3] w-full rounded-lg p-3 text-sm sm:text-base transition-all focus:ring-2 focus:ring-[#009FE3]/30 outline-none resize-none h-32"
            ></textarea>
...
const area = areas.find((a) => a.id === Number(route.params.id));
