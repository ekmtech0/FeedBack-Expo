<template>
    ...
    <div class="flex-grow flex items-center justify-center px-4 sm:px-8 py-10">
      ...
      >
        ...
        <div class="space-y-6">
          ...
          <div>
            <button
              class="w-full bg-[#009FE3] hover:bg-[#007CC0] text-white font-semibold py-3 rounded-lg transition-all duration-300 shadow hover:shadow-lg"
            >
              Enviar Feedback
            </button>
          </div>
    ...
    <MainHome v-if="Apareceroculto" @area-enviada="AreaRecebida()" />
...
const Apareceroculto = ref(true)
 const recerAreas = ref()

function AreaRecebida(nome) {
    console.log('Mensagem recebida do filho:', nome);
    recerAreas.value = nome

}

 console.log('Mensagem recebida do filho:', recerAreas);
</script>